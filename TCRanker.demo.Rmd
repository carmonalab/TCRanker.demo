...
Firstly, install TCRanker as well as its dependency [UCell](https://bioconductor.org/packages/release/bioc/html/UCell.html) and [scGate](https://github.com/carmonalab/scGate).
...
And we'll need some other packages to complete the workflow:
```{r}
library(Seurat)
```
...
need to be specified. It's also possible to present the clonotype as a separated 
...
sequentially, sub-divide the entry by the group, (i.e. the same clonotype might 
be divided into two or more sub-group and listed separately in the output if the
cells with the same TCR clonotype are from different groups.)
...
This is the output with group. You should have noticed that, despite of having
group to sub-divide the same clonotype, the number of entries in table 3 is
even smaller than table 2. This is due to `minClonSize=5` setting as default.
When sub-divided into smaller clonal groups, some of them might fall below the
threshold of 5 cells to be considered as meaningfully expanded. The number is
changeable.
...
The default signatures should be in the 
...
There should be plenty of tool available. For the demonstration of how to 
